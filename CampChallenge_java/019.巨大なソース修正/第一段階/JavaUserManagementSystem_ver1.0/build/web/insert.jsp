    ...
    <body>
        <form action="insertconfirm" method="POST">
            ...
            <select name="month">
                ...
                <%
                for (int i = 1; i <= 12; i++) {%>
                <option value="<%=i%>"<%if(udb != null){ %><%= udb.getMonth1()%>selected<% }%>><%= i%></option>
                <% } %>
            </select>月
            <select name="day">
                <option value="">----</option>
                <%
                    for (int i = 1; i <= 31; i++) {%>
                    <option value="<%=i%>"<%if(udb != null && i == Integer.parseInt(udb.getDay())){ %><%= Integer.parseInt(udb.getDay())%>selected<% }%>><%=i%></option>
                <% } %>
            </select>日
            <br><br>
            ...
            電話番号:
            <input type="tel" name="tell" maxlength="11" value="<%if (udb != null) {%><%=udb.getTell()%><%}%>">
            <br><br>
