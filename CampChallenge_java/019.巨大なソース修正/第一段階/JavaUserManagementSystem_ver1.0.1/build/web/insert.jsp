    ...
    <body>
        <form action="insertconfirm" method="POST">
            ...
            <select name="year">
                ...
                <option value="<%=i%>"<%if (udb != null) {%><%= Integer.parseInt(udb.getYear())%> selected <% }%>><%=i%> </option>
                <% } %>
            </select>年
            <select name="month">
                <option value="">----</option>
                <%
                    for (int i = 1; i <= 12; i++) {%>
                <option value="<%=i%>"<%if (udb != null) {%><%= Integer.parseInt(udb.getMonth())%> selected <% }%>><%= i%></option>
                <% } %>
            </select>月
            <select name="day">
                <option value="">----</option>
                <%
                    for (int i = 1; i <= 31; i++) {%>
                <option value="<%=i%>"<%if (udb != null) {%><%= Integer.parseInt(udb.getDay())%> selected <% }%>><%=i%></option>
                <% } %>
            </select>日
            <br><br>
