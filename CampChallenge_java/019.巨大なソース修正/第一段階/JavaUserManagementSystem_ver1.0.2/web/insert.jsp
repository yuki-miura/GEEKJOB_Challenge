    ...
    <body>
        <form action="insertconfirm" method="POST">
            ...
            <select name="year">
                ...
                %>
                <option value="<%=i%>" <%if (udb != null && udb.getYear().equals(y)) {%><%=Integer.parseInt(udb.getYear())%> selected <% }%> ><%=i%></option>
                <% } %>
            </select>年
            <select name="month">
                <option value="">--</option>
                ...
                %>
                <option value="<%=i%>" <%if (udb != null && udb.getMonth().equals(m)) {%><%=Integer.parseInt(udb.getMonth())%> selected <% }%> ><%= i%></option>
                <% } %>
            </select>月
            <select name="day">
                <option value="">--</option>
                ...
                %>
                <option value="<%=i%>" <%if (udb != null && udb.getDay().equals(d)) {%><%=Integer.parseInt(udb.getDay())%> selected <% }%> ><%=i%></option>
                <% } %>
            </select>日
            <br><br>
            ...
            電話番号:
            <input type="tel" name="tell" maxlength="13" value="<%if (udb != null) {%><%=udb.getTell()%><%}%>" placeholder="-を付けてください">
            <br><br>
