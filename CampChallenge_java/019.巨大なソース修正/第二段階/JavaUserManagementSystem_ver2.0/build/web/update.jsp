    ...
    <body>
        <form action="UpdateResult" method="POST">
            ...
            <select name="year">
                ...
                <option value="<%=i%>" <%if (i == birthDay.get(Calendar.YEAR)) {%> selected <%}%>><%=i%></option>
                <% } %>
            </select>年
            <select name="month">
                <option value="">--</option>
                <% for (int i = 1; i <= 12; i++) {%>
                <option value="<%=i%>" <%if (i - 1 == birthDay.get(Calendar.MONTH)) {%> selected <%}%>><%=i%></option>
                <% } %>
            </select>月
            <select name="day">
                <option value="">--</option>
                <% for (int i = 1; i <= 31; i++) {%>
                <option value="<%=i%>"<%if (i == birthDay.get(Calendar.DAY_OF_MONTH)) {%> selected <%}%>><%=i%></option>
                <% } %>
            </select>日
            <br><br>
        ...
        <p>
            <a href="resultdetail.jsp" style="width:100px">詳細画面へ戻る</a>
        </p>
        <p>
            <%=jh.home()%>
        </p>
